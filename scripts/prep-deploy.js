 ...
 *
 * 前置：先在项目根目录执行 `npm run build:h5` 生成最新产物。
 */
...
(async () => {
  ...
  const db = new Database(path.join(ROOT, 'server', 'server', 'data', 'gushi.sqlite'), { readonly: true, fileMustExist: true });
  ...
  // 2) 后台
  fs.copyFileSync(path.join(ROOT, 'server', 'package.json'), path.join(deploy, 'package.json'));
  fs.copyFileSync(path.join(ROOT, 'server', '.env'), path.join(deploy, '.env'));
  for (const file of fs.readdirSync(path.join(ROOT, 'server', 'server'))) {
    if (file.endsWith('.js')) fs.copyFileSync(path.join(ROOT, 'server', 'server', file), path.join(deploy, 'server', file));
  }
  const jwt = path.join(ROOT, 'server', 'server', 'data', 'jwt.secret');
  ...
  if (fs.existsSync(sharedAssets)) fs.cpSync(sharedAssets, path.join(deploy, 'public', 'assets'), { recursive: true });

  // 3) 小程序 H5 产物
  const distH5 = path.join(ROOT, 'miniapp', 'dist-h5');
  if (!fs.existsSync(path.join(distH5, 'index.html'))) {
    console.error('缺少 miniapp/dist-h5/index.html，请先在项目根目录执行 npm run build:h5');
    process.exit(1);
  }
  ...
  const checks = [
    ...
    ['品牌资源 logo.jpg', fs.existsSync(path.join(deploy, 'public', 'assets', 'logo.jpg'))],
