...
| 角色 | 主要权限 |
|---|---|
| 菇农 farmer | 基地/批次/设备/环境/预警/溯源/任务 读写，供应与采购只读，可用 AI 问答 |
| 合作社/基地管理员 base | 除账号管理外几乎全部读写 |
| 专家 expert | 专家问答读写、设备只读，其余只读 |
| 采购商 buyer | 发布采购需求，浏览供应与溯源，可用 AI 问答，无设备权限 |
| 政府/服务机构 government | 全部只读 + 统计（含设备与监测数据） |
...
│   │   └── forms.ts            # 10 类录入表单配置（含大棚设备，字段与后台表结构一致）
...
│       ├── monitor/            # 监测：大棚设备接入与在线状态、自动上报、手动补录、预警处理
│       ├── trace/              # 溯源：批次编号/扫码查询、时间线
│       ├── market/             # 市场：供应、采购需求、产品图标
│       └── expert/             # AI 智能问答：提问即得大模型回答，人工专家可补充
```
...
1. 在「监测」页点 **接入大棚设备**（或首页 ＋ → 大棚设备），填写设备名称、所属基地、阈值，可自定义设备编号。
2. 创建后会弹出 **设备接入凭证**（设备编号 + 设备密钥），把它们配置到温室网关（ESP32 / 智能网关）。
3. 硬件定时上报即可，数据自动出现在列表里，页面每 30 秒自动刷新：
...
4. 超过阈值自动生成预警；超过 10 分钟没有上报，设备显示为 **离线**。
5. 设备卡片可展开 **近 24 小时温度曲线**（按 2 小时分桶的平均值），并显示平均/最低/最高与记录条数。
6. 也支持 MQTT 接入：把设备报文发到 `<前缀>/devices/<设备编号>/readings`，后台 `npm run mqtt` 适配层会自动入库（详见 `后台/README.md`）。
7. 「手动补录」只用于断网、设备维修或核对历史数据，记录会标注来源（硬件自动 / 手动补录）。

## 七、AI 智能问答

- 「AI 问答」页输入问题即可，回答会自动带上本账号最近的基地、批次与环境数据作为上下文。
- 回答带来源标记：**AI 大模型**（`source=ai`）/ **规则知识库**（`source=rule`，未配置 API Key 或调用失败时降级）/ **人工专家**。
- 所有问答自动存档，可在页面下方查看，也能由人工专家补充或纠正。
- 后台在 `后台/.env` 配置 `AI_BASE_URL` / `AI_API_KEY` / `AI_MODEL`（当前使用 DeepSeek `deepseek-chat`），改完重启后台即可，小程序无需改动。
...
| FR-03 环境录入 | **大棚设备自动上报**（`POST /api/ingest/readings`），监测页展示设备与在线状态；断网等场景可“手动补录” |
| FR-04 自动预警 | 设备上报后按阈值自动生成，监测页/首页可“处理”，POST `/api/alerts/:id/ack` |
| FR-05 专家问答 | **AI 智能问答**（`POST /api/ai/ask`），未配置大模型时降级规则知识库，人工专家可补充回复 |
...
| FR-11 权限控制 | JWT + RBAC（6 种角色，含设备权限），前端隐藏 + 后端强制 |
| FR-12 编辑删除 | 小程序端提供新增与图标/回复更新；完整编辑删除在后台扩展 |
| FR-14 微信小程序 | 本工程，五个 Tab + AI 问答页 |
...
5. 环境数据以**大棚设备自动上报**为准，手动补录会标注来源；AI 回答为辅助建议，必须标注来源（AI / 规则知识库 / 人工专家），重要决策以当地农技专家意见为准。
